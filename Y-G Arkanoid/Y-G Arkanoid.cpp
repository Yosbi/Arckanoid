...
ArkanoidGame* gArkGame = 0;
AirHockeyGame* gAirHockey  = 0;

string gWndCaption = "Y-Games - Arkanoid";
...
const int gClientWidth = 625;
const int gClientHeight = 650;
...
bool InitMainWindow()
{
	...
	ghMainWnd = ::CreateWindow("MyWndClassName", 
		...
		200, 50, gWindowWidth, gWindowHeight, 0, 
	...
	while(msg.message != WM_QUIT)
	{
        ...
        {	
			...
			{ 				
					...
					gArkGame->ArkMain((float)timeElapsed, gBackBuffer->getDC(), ghSpriteDC, gClientCenter);
			...
			// the end of the loop iteration, so
...
{	
	...
	case WM_LBUTTONDOWN:
		if((gArkGame->looser()) || (gArkGame->mfinished)){
			delete gArkGame;
			gArkGame = new ArkanoidGame(
			ghAppInst, hWnd, gClientCenter);
		}
