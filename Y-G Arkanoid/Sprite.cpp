...
void Sprite::update(float dt)
{
	...
	//update bounding rect too. That is, the bounding 
	// rect moves with the sprite too.
	int w = width();
	int h = height();

	int x0 = (int)mPosition.x - (w / 2);
	int y0 = (int)mPosition.y - (h / 2);
	int x1 = (int)mPosition.x + (w / 2);
	int y1 = (int)mPosition.y + (h / 2);
	mBoundingRect.minPt.x = (float)x0;
	mBoundingRect.minPt.y = (float)y0;
	mBoundingRect.maxPt.x = (float)x1;
	mBoundingRect.maxPt.y = (float)y1;
}
