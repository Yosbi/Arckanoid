...
string gWndCaption = "Y-Games - Air Hockey";
...
{	
	...
	case WM_COMMAND:
		...
		case ID_HELP_ABOUT:
			...
			break;
			//change difficlut, changes the red paddle speed
		case ID_DIFFICULT_EASY:
			gAirHockey->RED_SPEED = 300.0f;
			break;
		case ID_DIFFICULT_HARD:
			gAirHockey->RED_SPEED = 600.0f;
			break;
		}
	...
	case WM_LBUTTONDOWN:
		if(gAirHockey->mPaused == true)
			gAirHockey->unpause();
		return 0;

	// Right mouse button to pause the game.
	case WM_RBUTTONDOWN:
		if(gAirHockey->mPaused == false)
		gAirHockey->pause();
		return 0;
